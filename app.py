...
<style>
    ...
    .custom-title {
        ...
        background: linear-gradient(135deg, #3D1F12, #584738);
    ...
    .analysis-card-item {
        background: linear-gradient(145deg, #584738, #3D1F12);
        border-radius: 15px;
        padding: 1.5rem;
        width: 300px;
        height: 180px;
    ...
    .analysis-card-item:hover {
        ...
        background: linear-gradient(145deg, #7A5E47, #584738);
    }
    ...
    .btn-espresso .stButton > button {
        background: linear-gradient(135deg, #3D1F12, #2A1308);
    }
    .btn-espresso .stButton > button:hover {
        background: linear-gradient(135deg, #2A1308, #1A0B05);
        transform: translateY(-2px);
        box-shadow: 0 6px 20px rgba(61, 31, 18, 0.4);
    }
    
    .btn-mahogany .stButton > button {
        background: linear-gradient(135deg, #584738, #403226);
    }
    .btn-mahogany .stButton > button:hover {
        background: linear-gradient(135deg, #403226, #2D2419);
        transform: translateY(-2px);
        box-shadow: 0 6px 20px rgba(88, 71, 56, 0.4);
    }
    
    .btn-light-brown .stButton > button {
        background: linear-gradient(135deg, #98755B, #7A5E47);
    }
    .btn-light-brown .stButton > button:hover {
        background: linear-gradient(135deg, #7A5E47, #5C4635);
        transform: translateY(-2px);
        box-shadow: 0 6px 20px rgba(152, 117, 91, 0.4);
    }
    
    .btn-milk .stButton > button {
        background: linear-gradient(135deg, #F1EADA, #E8DCC0);
        color: #3D1F12 !important;
        border: 2px solid #98755B;
    }
    .btn-milk .stButton > button:hover {
        background: linear-gradient(135deg, #E8DCC0, #DFD1B0);
        transform: translateY(-2px);
    ...
    .stTextArea > div > div > textarea {
        border-radius: 15px;
        border: 2px solid #98755B !important;
    ...
    /* Sidebar styling */
    .css-1d391kg {
        background: linear-gradient(180deg, #F1EADA, #FFFFFF);
    }
    
    /* File uploader styling - Dark theme */
    .stFileUploader > div > div {
        background: rgba(45, 36, 25, 0.9) !important;
        border: 2px dashed #98755B !important;
        border-radius: 15px;
        padding: 2rem;
        color: #F1EADA !important;
    }
    ...
    /* Response container */
    .response-container {
        background: linear-gradient(145deg, #F9F7F4, #F1EADA);
        border-radius: 15px;
        padding: 2rem;
        margin-top: 2rem;
        border-left: 5px solid #98755B;
...
st.sidebar.markdown("""
<div style="background: linear-gradient(145deg, #F1EADA, #FFFFFF); padding: 1.5rem; border-radius: 15px; margin-bottom: 1rem;">
    <h4 style="color: #3D1F12; margin-bottom: 1rem;">📋 Steps to Follow:</h4>
    <ol style="color: #584738;">
...
for i, option in enumerate(analysis_options):
    ...
    with cols[col_idx]:
        ...
        <div class="analysis-card-item" onclick="document.getElementById('{option["key"]}').click();">
            ...
            <h3 style="color: #F1EADA; margin-bottom: 0.5rem; font-family: 'Inter', sans-serif;">{option["title"]}</h3>
            <p style="color: #98755B; font-size: 0.9rem; margin: 0; font-family: 'Inter', sans-serif;">{option["desc"]}</p>
        </div>
        """
        st.markdown(card_html, unsafe_allow_html=True)
        
        # Hidden button for functionality (no help text to avoid blue boxes)
        if st.button("", key=option["key"]):
...
def generate_response(prompt):
    if uploaded_file is not None:
        ...
        st.warning("⚠️ Please upload your resume to proceed with the analysis!")
